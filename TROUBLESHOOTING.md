...
This is likely due to the ratio of monitored devices to orker threads being too high.  Increase the number of worker threads using `--threads` or increase the scan period `--scan-period`.  If the issue only arises when device(s) are downed, you can try the methods mentioned before, or you can try decreasing the timeout period `--device-timeout`.
...
Try restarting Kibble.  This will force Kibble to update it's device properties.
