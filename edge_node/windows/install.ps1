...
& "$VenvDir\Scripts\python.exe" -m pip install -r "$ScriptDir\requirements-windows.txt"

# Copy collector scripts into the install directory
Copy-Item "$ScriptDir\..\KibbleDaemon" $InstallDir -Recurse -Force
Copy-Item "$ScriptDir\..\main.py" $InstallDir -Force
