...
class SCPIMonitor(StatusMonitor):
    ...
    def __init__(
        ...
        """
        super().__init__(endpoints or [], timeout)
    ...
    def _get_scpi_idn(self, target: str) -> Optional[str]:
        """
    ...
    async def _send_idn_await_reply(self, target: str) -> tuple[bool, int, int]:
        ...
        try:
            ...
            self.maintainance_logger.critical(
                f"Could not resolve the hostname for {target}"
            )
        ...
        response = await loop.run_in_executor(
            self._executor, lambda: self._get_scpi_idn(ip)
        )
        ...
        return (
            True,
            round((response_time - request_time) * 1000),
